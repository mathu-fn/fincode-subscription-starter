...
API は **JWT Bearer認証** を使います。JWT は Google ログインで発行します。ブラウザの Google Identity Services (GIS) ボタンで取得した ID トークン（credential）をバックエンドへ渡すと、検証のうえアプリ独自の JWT が返ります。

```http
POST /api/auth/google
Content-Type: application/json

{ "credential": "<GIS が返す Google ID トークン>" }
```
...
| メソッド | パス | 認証 | レート制限 | 説明 |
| --- | --- | --- | --- | --- |
| POST | `/api/auth/google` | 不要 | 5回/分 | Google IDトークンでログインしJWT発行（初回はユーザー作成） |
...
| DELETE | `/api/subscription` | 必須 | 5回/分 | サブスクリプション解約（有料契約は期間末まで利用可能） |
...
| --- | --- |
| 401 | JWTなし、期限切れ、不正 |
| 404 | リソースなし（他ユーザーのリソースも ID 列挙防止のため 404 で応答） |
| 409 | 状態競合 |
| 422 | 入力または業務ルールのエラー |
| 429 | レート制限 |
