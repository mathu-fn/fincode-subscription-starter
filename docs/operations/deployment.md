...
fincode からの定期課金結果 Webhook は FastAPI プロセスが同期で処理します。バックグラウンドワーカーは本スターターには含まれていません。Webhook ハンドラは署名検証 → 冪等性チェック → DB upsert を 1 リクエスト内で完結させて 204 を返します。

通知配信や下流プロビジョニングなど重い副作用を導入する場合は、fork 側で別途キュー / ワーカーを追加してください。
